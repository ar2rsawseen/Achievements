...
function TextWrap:setText(text)
	...
	for i = 1, iters do
		...
		if self.align == "left" or (newLine and self.align == "justify") then
			self:addChild(line)
			line:setPosition(0, math.floor(height))
		elseif self.align == "right" then
			self:addChild(line)
			line:setPosition(math.floor(self.width - line:getWidth()), math.floor(height))
		elseif self.align == "center" then
			self:addChild(line)
			line:setPosition(math.floor((self.width - line:getWidth())/2), math.floor(height))
		elseif self.align == "justify" then
			...
			if diff == 0 or i == iters then
				...
				for wordString in part:gmatch("[^%s]+") do 
					...
					self:addChild(word)
					word:setPosition(math.floor(lastPos), math.floor(height))
